...
abiDecoder.addABI(EnumAbi[process.env.CHAIN_ID].ROUTERS.NEXISSWAP);
...
class Scraper {
    ...
    async getTokens(pairAddress, cachedPair){
        ...
        let token1 = 0;
        try {
            let pairWeb3Contract =  await new this.web3.eth.Contract( EnumAbi[process.env.CHAIN_ID].PAIR.NEXISSWAP, pairAddress );
            if( !cachedPair ){ 
    ...
    async loopUpdateMainTokenPrice(){
        ...
        while( true ){
            try {
                ...
                let mainTokenPair = await new this.web3.eth.Contract( EnumAbi[process.env.CHAIN_ID].PAIR.NEXISSWAP, mainTokenPairAddress );
