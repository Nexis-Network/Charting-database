...
class Router {
    ...
    async getFeePancakeSimilar(routerAdd){
        let routerContract = await new this.web3.eth.Contract( EnumAbi[EnumChainId.NZT].ROUTERS.NEXISSWAP, routerAdd );
    ...
    async getFeeBiswapSimilar(routerAdd, pairAdd){
        ...
        if( !pairAdd ) return -1;
        let routerContractBiswap = await new this.web3.eth.Contract( EnumAbi[EnumChainId.NZT].ROUTERS.BISWAP, routerAdd );
        let factory = await routerContractBiswap.methods.factory().call();
        let pairContract = await new this.web3.eth.Contract( EnumAbi[EnumChainId.NZT].PAIR.BISWAP, pairAdd );
