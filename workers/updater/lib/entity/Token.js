...
class Token {
    ...
    async getToken( token ){
        ...
        if( searchOnDb && (!tokenInfo || tokenInfo.notFound) ) {
            ...
            if(!tokenInfo) {
                let token_contract = await new this.web3.eth.Contract( EnumAbi[EnumChainId.NZT].TOKEN, token );
                let token_decimals;
                let name;
                let supply;
                let symbol;
    ...
    async getBurned( token ){
        try {
            let tokenContract = await new this.web3.eth.Contract( EnumAbi[EnumChainId.NZT].PAIR.NEXISSWAP, token );
